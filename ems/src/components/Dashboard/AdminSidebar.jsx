...
const AdminSidebar = () => {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center bg-teal-600 h-[8vh] px-3 ${expanded ? "justify-between" : "justify-center"}`}
      >
        <button onClick={() => setIsOpen(!isOpen)} className="text-white">
          {isOpen ? <CloseIcon /> : <MenuIcon />}
        </button>
        {expanded && <h3 className="text-2xl truncate text-white text-opacity-90">Employee MS</h3>}
      </div>
